...
def convert_to_sarif(input_file, output_file, commit_hash):
    ...
    Changes to match your request:
      - rule.name = "<pkg_name>==<pkg_version>" => shown as the alert "title" in GitHub
      - shortDescription = "Alert generated by Socket Security"
      - fullDescription = big chunk of text: includes alert["title"], alert["description"], alert["suggestion"], props["note"], commit
      - result.message.text = the same chunk, so it shows up in the main code scanning comment as well.
    """
    ...
    for idx, alert in enumerate(alerts, start=1):
        ...
        # We'll build one big chunk of text from the alert:
        title       = alert.get("title", "")
        description = alert.get("description", "")
        suggestion  = alert.get("suggestion", "")
        note_text   = alert.get("props", {}).get("note", "")

        # Merge them all with blank lines in between
        # so it looks good in the GitHub code scanning comment
        message_parts = [
            title,
            description,
            suggestion,
        ...
        message_parts = [x for x in message_parts if x.strip()]
        combined_message = "\n\n".join(message_parts)

        # Determine the manifest file & snippet
        ...
        # If we haven't seen this rule_id yet, define it
        if rule_id not in rules_map:
            # The rule name => "<pkg_name>==<pkg_version>", e.g. "maling==2.2.1"
            rule_obj = {
                "id": rule_id,  # e.g. "malware"
                "name": f"{pkg_name}=={pkg_version}",
                "shortDescription": {
                    # So GitHub shows "Alert generated by Socket Security"
                    "text": "Alert generated by Socket Security"
                },
                "fullDescription": {
                    # Replace the "Additional details..." placeholder with your chunk
        ...
        # Build the SARIF result referencing the same chunk in the "message"
        result_obj = {
            "ruleId": rule_id,
            # This is what appears as the main comment in GitHub code scanning
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert Socket results to SARIF with updated titles & details placement."
    )
    ...
    parser.add_argument("--commit_hash", default=None, help="Optional commit hash to include in the final message.")
