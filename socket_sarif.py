...
def convert_to_sarif(input_file, output_file, commit_hash):
    ...
    Requirements:
      - rule.name = "<pkg_name>==<pkg_version>" => becomes the top-line title
      - shortDescription = "Alert generated by Socket Security"
      - fullDescription = big chunk with [title, description, suggestion, note, commit]
      - result.message.text = short / minimal, so the big chunk is only in the rule's fullDescription
    """
    ...
    for idx, alert in enumerate(alerts, start=1):
        ...
        # Large chunk for the "fullDescription"
        note_text   = alert.get("props", {}).get("note", "")
        big_chunk_parts = [
            alert.get("title", ""),        # e.g. "Known malware"
            alert.get("description", ""),  # e.g. "This package is malware..."
            alert.get("suggestion", ""),   # e.g. "It is strongly recommended..."
            note_text,                     # The long note
            f"Commit: {commit_hash}",
        ]
        # Filter out empty strings
        big_chunk_parts = [x for x in big_chunk_parts if x.strip()]
        big_chunk = "\n\n".join(big_chunk_parts)
        ...
        # This is where we set the "title" to be "pkg_name==pkg_version"
        if rule_id not in rules_map:
            rule_obj = {
                "id": rule_id,  
                "name": f"{pkg_name}=={pkg_version}",  # top-line alert title
                "shortDescription": {
        ...
        # The main "comment" text is short, so the big chunk only appears in fullDescription
        result_obj = {
            "ruleId": rule_id,
            "message": {
                "text": "See the extended details in the rule's full description."
            },
            "locations": [
                {
                    "physicalLocation": {
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert Socket results to SARIF with pkg_name==pkg_version as the top-line title."
    )
    ...
    parser.add_argument("--commit_hash", default=None, help="Optional commit hash to place in the fullDescription.")
