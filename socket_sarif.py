...
def convert_to_sarif(input_file, output_file, commit_hash):
    ...
    - The rule name is set to the alert's 'purl' (e.g. "pypi/maling@2.2.1").
    ...
    for idx, alert in enumerate(alerts, start=1):
        ...
        purl        = alert.get("purl", "no_purl_available")

        # Package info for snippet location
        ...
        # If we haven't seen this 'ruleId' before, define the rule
        if rule_id not in rules_map:
            # Use the purl as the top-level "name," so GitHub’s code scanning title is the purl
            # e.g. "pypi/maling@2.2.1"
            rule_obj = {
                "id": rule_id,            # e.g. "malware"
                "name": purl,            # e.g. "pypi/maling@2.2.1"
                "shortDescription": {
                    # Could be minimal or reference the 'title'. 
                    "text": "Alert generated by Socket Security"
                },
                "fullDescription": {
                    # You could leave this empty or add some placeholder:
                    "text": "Additional details appear in the alert message."
                },
        ...
        # Build the main message that appears in GitHub's code-scanning comment
        # We combine the Socket 'title', 'description', 'suggestion', 'note', and commit hash:
        message_parts = [
            title,
            description,
            suggestion,
        ...
        # Filter out any empty lines so we don't get extraneous blank spacing
...
def main():
    parser = argparse.ArgumentParser(description="Convert Socket results to SARIF with the purl as the rule title.")
